...
| API and CLI control | The current file-backed CLI can drive both the ascension prototype and a first conquest-victory prototype headlessly, and the local HTTP API now supports session create, step, command submission, player-scoped state, player-scoped events, save/load, and metrics; the remaining gap is converging the CLI and server surfaces so manual playtesting exercises the same path | API and CLI productization, Hardening and balance harness |
