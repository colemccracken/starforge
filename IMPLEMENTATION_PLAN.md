...
| API and CLI control | The file-backed CLI can drive both the ascension prototype and a first conquest-victory prototype headlessly, the local HTTP API now supports session create, step, command submission, player-scoped state, player-scoped events, save/load, and metrics, and the CLI can now drive those same flows through `--api-base`; the active remaining gap is route-aware map parity in API mode, with run/pause semantics now a lower-priority follow-up decision | API and CLI productization, Hardening and balance harness |
