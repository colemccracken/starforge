...
- Current state: the CLI can now drive the prototype through either file-backed or HTTP-backed transport, and the HTTP API surface covers session creation, stepping, player-scoped state and events, save/load, metrics, and route-aware map output. The main remaining mismatch in this milestone is that `planned_routes()` still advertises `run` and `pause` control semantics that are not yet implemented.
...
| API and CLI control | The file-backed CLI can drive both the ascension prototype and a first conquest-victory prototype headlessly, the local HTTP API now supports session create, step, command submission, player-scoped state, player-scoped events, save/load, metrics, and route-aware map reads, and the CLI can now drive those same flows through `--api-base`; the active remaining mismatch is that explicit `run` and `pause` control routes are still planned but not yet implemented | API and CLI productization, Hardening and balance harness |
