...
- Current state: the CLI can now drive the prototype through either file-backed or HTTP-backed transport, and the HTTP API surface covers session creation, run, pause, step, command submission, player-scoped state and events, save/load, metrics, and route-aware map output. The CLI now also exposes direct metrics, save/load wrappers, and a deterministic file-backed `scenario-run` flow, so Milestone 9 is considered complete.
...
| API and CLI control | The file-backed CLI can drive both the ascension prototype and a first conquest-victory prototype headlessly, the local HTTP API now supports session create, run, pause, step, command submission, player-scoped state, player-scoped events, save/load, metrics, and route-aware map reads, and the CLI can now drive those same flows through `--api-base` while also exposing direct metrics, persistence wrappers, and deterministic scenario-runner setup | API and CLI productization, Hardening and balance harness |
