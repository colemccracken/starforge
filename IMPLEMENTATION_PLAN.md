...
Status: Active plan; Milestones 1-3 complete, Milestone 4 in progress  
Last Updated: 2026-03-25  
Source of Truth: [STARFORGE_REFERENCE.md](STARFORGE_REFERENCE.md)  
Current Milestone: Economy and infrastructure  
Current Focus: Extend the economy slice from repair-driven recovery into proactive build queues, infrastructure expansion, and broader resource-spending behavior.

## Current Execution State

- Current milestone: Economy and infrastructure
- Current next action: Add construction queues for new infrastructure so local build capacity and stockpiles can expand energy, datacenter, relay, and extraction footprints instead of only restoring damaged assets.
- Blockers: None currently identified.
- Recently completed: Bootstrapped the Rust workspace, added the six planned crates, created baseline tooling with `Makefile`, added placeholder content and scenario files, then completed the simulation-foundation milestone by implementing deterministic pending-command scheduling, persisted replay and pending command data in snapshots, added snapshot restore and replay reconstruction support, added in-memory plus JSON snapshot round-trip tests, replaced the placeholder command path with deterministic throughput and agent-assignment mutations, added seeded RNG state with persistence and regression coverage in `starforge-core`, introduced structured command and domain event payloads, added deterministic location and relay mutations, added a save/load continuation equivalence regression, completed the world-and-content milestone by replacing placeholder YAML parsing in `starforge-content` with typed ruleset and scenario documents, generating deterministic homeworld and neutral starting locations from content data, wiring `GameSession` bootstrap to honor generated starting locations from `ScenarioConfig`, adding file-backed scenario harness loading plus session instantiation in `starforge-scenarios`, adding typed planetary attributes and deterministic location connections so scenarios carry a basic solar-system topology into session state, remediating the old placeholder bootstrapping drift by routing API startup through the canonical compiled scenario path and renaming the default core scenario shape into an explicit test fixture, adding typed homeworld infrastructure packages plus structured hostile-remnant seed data to generated worlds, starting the economy-and-infrastructure milestone with runtime infrastructure state, computed energy and datacenter throughput, relay isolation effects, and economy-driven throughput budget validation in `starforge-core`, extending that economy slice with seeded stockpile initialization, per-location extraction output computation, connected stockpile aggregation, tick-based extraction accumulation, deterministic infrastructure wear thresholds, condition-change events, and automatic economy degradation as infrastructure falls into degraded or offline states, then adding resource-spending repair queues that consume local or connected stockpiles, progress deterministically through tick time, and restore degraded infrastructure capacity on completion.
...
| Economy and infrastructure | In Progress | Implement extraction, energy, datacenters, maintenance, relays, and throughput | Disconnected worlds and power deficits behave per design | Simulation foundation, World and content pipeline |
...
| Progression and agents | Not Started | Implement research, training, model tiers, and non-LLM agent orchestration | Progression and automation pressure the economy correctly | Economy and infrastructure, Movement, intel, and control, Military and victory state contracts |
...
| API and CLI productization | Not Started | Finalize local control surfaces, metrics, and scenario runner ergonomics | Both empires can be driven end to end through API or CLI | Simulation foundation, Movement, intel, and control, Military and victory, Progression and agents, Engine-managed local inference |
...
| Visibility correctness | Contested worlds reveal major actions only; stale and observed information states project correctly by player | Movement, intel, and control; API and CLI productization; Hardening and balance harness |
| Combat and conquest | Hostile remnant expansion can fail if underprepared; captured worlds transfer damaged infrastructure; destroyed worlds remain unusable | World and content pipeline, Military and victory, Hardening and balance harness |
| Victory resolution | Ascension can be interrupted by contest, relay loss, or throughput loss; no-draw enforcement always resolves to one winner | Military and victory, Hardening and balance harness |
...
| API and CLI control | Both empires can be driven headlessly through API or CLI without a built-in bot | API and CLI productization, Hardening and balance harness |
